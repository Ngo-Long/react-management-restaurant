...
const OrderCard: React.FC<OrderCardProps> = ({ orderItems, onRemoveItem, onChangeQuantity }) => {
    ...
    const totalPrice = orderItems.reduce((sum, item) => sum + item.sellingPrice * item.quantity, 0);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (value: number, record: any, index: number) => (
                <>
                    <MinusOutlined
                        style={{ background: 'red', color: 'white', padding: '2px', borderRadius: '4px', cursor: 'pointer' }}
                        onClick={() => onChangeQuantity(record.id, -1)}
                    />
                    &nbsp;  &nbsp;
                    {value}
                    &nbsp;  &nbsp;
    ...
    return (
        ...
        >
            <div className="container" style={{ height: 'calc(100vh - 96.5px)' }}>
                <div style={{ height: '70%' }}>
                    {orderItems.length === 0 ? (
                        <div style={{
                            display: 'flex',
                ...
                <div >
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', margin: '10px 0', fontWeight: 600 }}>
                        <div style={{ fontSize: '16px', color: '#393939' }}>
                            Tổng tiền
                            <span style={{ color: '#393939', fontSize: '15px' }}>&nbsp;({orderItems.length} món)</span>
                        </div>

                        <div style={{ color: 'red', fontSize: '18px' }}>{totalPrice.toLocaleString()} đ</div>
                    </div>
